...
Vector<T>::Vector(Vector<T> const &tmp_) {
    ...
    arr_ = reinterpret_cast<T*>(new int8_t[size_ * sizeof(T)]);//reserve(capacity_);
...
Vector<T>::~Vector() {
    ...
    delete[] reinterpret_cast<int8_t *>(arr_);//это не факт что надо
    arr_ = nullptr;
    capacity_ = 0;
    size_ = 0;
}
...
void Vector<T>::erase(Vector<T>::b_iterator beg, Vector<T>::b_iterator end) {
    ...
    for (auto i = i1; i <= i2; i++) { // <
        (arr_ + i)->~T();
        new(arr_ + i) T(arr_[i + 1]);// вместе 1[i2-i1]
    }
    size_ = size_ - (i2 - i1);
}
//
...
typename Vector<T>::r_iterator Vector<T>::rbegin() {
    return r_iterator(arr_ + size_);//-1
}

template<typename T>
typename Vector<T>::r_iterator Vector<T>::rend() {
    return r_iterator(arr_);//-1
}
