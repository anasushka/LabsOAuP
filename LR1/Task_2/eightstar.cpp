...
EightStar::EightStar(qreal outerWidth, qreal outerHeight,
...
void EightStar::setStar(qreal outerWidth, qreal outerHeight,
                       qreal innerWidth, qreal innerHeight)
{
...
double EightStar::perimeter() const
{
    double p = 0;
    QPolygonF poly = getPolygon();
    int n = poly.size();
...
QPointF EightStar::centerOfMass() const
{
    // При симметричном построении центр масс находится в (0,0)
    return QPointF(0, 0);
}

void EightStar::setCenterOfMass(const QPointF &newCenter)
{
   setTransformOriginPoint(newCenter);
}

QRectF EightStar::boundingRect() const
{
